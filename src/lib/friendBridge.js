...
import { db } from '../storage/firebaseClient.js'
import { dateStr, streak, treeInfo } from './util.js'

const PROGRESS_COL = 'planner_progress'
const CHEERS_COL = 'planner_cheers'
...
export async function pushProgress(data) {
  try {
    ...
    await setDoc(
      doc(db, PROGRESS_COL, snapshot.date),
...
export async function pullCheers(max = 5) {
  try {
    const q = query(collection(db, CHEERS_COL), orderBy('createdAt', 'desc'), limit(max))
