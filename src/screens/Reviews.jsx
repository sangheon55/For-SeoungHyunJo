...
export default function Reviews() {
  ...
  return (
    <div>
      ...
      {data.reviews.length === 0 && <div className="card empty">등록된 교재가 없어요.</div>}
      {data.reviews.map((b) => {
          ...
          <div className="card" key={b.id} style={{ marginBottom: 12 }}>
            ...
            <div className="flex-between">
              ...
              <div className="row" style={{ gap: 6 }}>
                <button className="btn ghost sm" onClick={() => step(b.id, -1)}>− 챕터</button>
                <button className="btn sm" onClick={() => step(b.id, 1)}>+ 챕터</button>
                <button className="btn danger sm" onClick={() => del(b.id)}>삭제</button>
              </div>
            </div>
          </div>
        )
