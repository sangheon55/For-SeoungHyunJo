...
const KAGUYA_ACCESS_TOKEN = defineSecret('KAGUYA_ACCESS_TOKEN')
const MODEL = 'gpt-4o'
...
꾸준한 순공시간 확보, 기출문제 점검, 답안 작성과 첨삭, 약점 과목 보완을 분석 기준으로 삼는다.
특정 합격자의 공부시간을 절대 기준처럼 단정하지 말고 기록의 꾸준함과 과목 균형을 우선 평가한다.
...
export const kaguya = onRequest(
  {
    region: 'asia-northeast3',
    secrets: [OPENAI_API_KEY, KAGUYA_ACCESS_TOKEN],
    timeoutSeconds: 30,
    memory: '256MiB',
  },
  async (req, res) => {
    ...
    if (req.get('X-Kaguya-Access') !== KAGUYA_ACCESS_TOKEN.value()) {
    ...
    try {
      ...
      if (req.body?.mode === 'dialogue') {
        const text = await createResponse({
          instructions: `${persona}
이 요청에서는 설명하지 말고 요청된 JSON만 출력한다.`,
          input: dialogueInput(req.body.context),
          maxOutputTokens: 180,
        })
        res.json({ dialogue: parseDialogue(text) })
        return
      }
      ...
      if (req.body?.mode === 'fujiwara-interrupt') {
        ...
        res.json({ beats: parseFujiwaraInterrupt(text) })
        return
      }
...
{"face":"neutral|serious|tired|smile","text":"이시가미 말투의 조언"}`,
        ...
        res.json({ advice: parseIshigamiAdvice(text) })
        return
      }
